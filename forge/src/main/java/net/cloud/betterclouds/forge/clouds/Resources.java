...
import net.cloud.betterclouds.forge.NamedLogger;
import net.cloud.betterclouds.forge.clouds.shaders.CoverageShader;
import net.cloud.betterclouds.forge.clouds.shaders.DepthShader;
import net.cloud.betterclouds.forge.clouds.shaders.ShadingShader;
import net.cloud.betterclouds.forge.compat.DistantHorizonsCompat;
import net.cloud.betterclouds.forge.compat.Telemetry;
import net.cloud.betterclouds.forge.mixin.BufferRendererAccessor;
import net.cloud.betterclouds.forge.mixin.ShaderProgramAccessor;
import net.cloud.betterclouds.forge.mixin.VertexBufferAccessor;
...
public class Resources implements Closeable {
    ...
    public void reloadFramebuffer(int width, int height) {
        if(width == 0 || height == 0) {
            Main.LOGGER.warn("Cannot create framebuffer with size 0 ({}x{})! Skipping framebuffer creation to avoid an error.", width, height);
            return;
        }
        deleteFramebuffer();
    ...
    protected void reloadShadersInternal(ResourceManager manager, boolean safeMode) throws IOException {
        ...
        coverageShader = CoverageShader.create(manager, Config.sizeXZ, Config.sizeY, edgeFade, glCompat.useStencilTextureFallback, DistantHorizonsCompat.instance().isReady() && DistantHorizonsCompat.instance().isEnabled());
        coverageShader.bind();
